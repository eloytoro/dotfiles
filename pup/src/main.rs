...
const SKIP_DIRS: &[&str] = &[".git", ".hg", ".svn", "node_modules"];

fn walk_files(start: &Path, visited: &HashSet<PathBuf>, gitignore: &Option<(Gitignore, PathBuf)>, exclude: &Option<PathBuf>, mut on_file: impl FnMut(PathBuf)) {
...
mod tests {
    ...
    fn collected_files(start: &Path, visited: &HashSet<PathBuf>, gitignore: &Option<(Gitignore, PathBuf)>, exclude: &Option<PathBuf>) -> Vec<PathBuf> {
    ...
    fn walk_files_finds_all_files_recursively() {
        ...
        assert_eq!(files, vec![root.join("a.txt"), root.join("b.txt"), root.join("sub/c.txt")]);
    }
    ...
    fn walk_files_direct_files_before_subdirectory_files() {
        ...
        let nested_pos = files.iter().position(|p| p == &root.join("sub/b.txt")).unwrap();
